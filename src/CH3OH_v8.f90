...
!  TAKING THIS OUT FOR NOW TO TRY AND MAKE MORE GENERAL (BPM)
 ...
 double precision  :: last_stadev,factor_back,factor_out,fit_scale=1.00d0,gamma_ps,enermax_ps,tol=0.1d0
 double precision  :: rms,ZPE,stadev_best,TEMPE,Ener_min
...
!---input energy values------!
 ...
 Ener_min = minval(energy(1:npts))*conv_energy
 ZPE = 0.0d0 
 !
 ...
 outer_loop: do while (still_run)  
   ...
   do while( iter<=itmax .and. stadev>stadev_best .and. (  stability.ge.stab_best) )   
     ...
     if (iter>10.and.factor_out<0) still_run = .false.
     !
     ssq=0.0d+00
     rms=0.0d+00
     !
     ...
     if (itmax.ne.0) then
      ...
      case ("SVD","svd")
         !
         call lapack_dgesvd(tol,rjacob(1:npts,1:numpar))
         !
...
!------------------------------------------!
  ...
  subroutine lapack_dgesvd(tol,h)
    ...
    double precision,allocatable    :: work(:),u(:,:),vt(:,:),s(:),s_inv(:),v(:,:),ut(:,:)
    integer           :: info
    integer           :: nh1, nh2,i,Nkeep,j
    integer           :: lwork
    ...
    lwork = 50*nh1
    !
    allocate(work(lwork),u(nh1,nh2),ut(nh2,nh1),vt(min(nh1,nh2),nh2),v(nh2,min(nh1,nh2)),s(min(nh1,nh2)),s_inv(min(nh1,nh2)),stat=info)
    !
    ...
    !
    call dgesvd(jobu,jobvt,nh1,nh2,h,nh1,s,u,nh1,vt,nh2,work,lwork,info)
    !
    ! pseudo-inverse 
    !
    Nkeep = 0
    ...
    !h = transpose(ut)
    !
    h(:,1:nh2) = matmul(u(:,1:Nkeep),vt(1:nkeep,1:nh2))
    !
    ...
    !
    deallocate(work,u,vt,ut,v,s,s_inv)
    !
    if (info/=0) then
