...
!---input energy values------!
 ...
 outer_loop: do while (still_run)  
   ...
   if (itmax.ne.0.and.find_best_params) then
     ...
     enddo 
     do i=nparam_eq+1,parmax
        write (f_out,"(a2,<ndeg>i3,i4,2x,f18.8)") trim(parnam(i)),ipower(1:NDEG,i),ivar(i),param(i)
     enddo 
...
!--  printing  out the resulted information 
  ...
  if (itmax.ne.0) then 
   ...
   enddo 
   do i=nparam_eq+1,parmax
      write (f_res,"(a2,<ndeg>i3,i4,2x,f18.8)") trim(parnam(i)),ipower(1:NDEG,i),ivar(i),param(i)
   enddo 
  endif  ! ---- itmax/=0
...
!ahh3 = 240.0+tbar+1.0/6.0*sqrt(6.0)*A1-1.0/6.0*sqrt(2.0)*A2  !12
  ...
  xi(12) = tau
  !
  call ML_symmetry_transformation_XY3_II(nsym,xi,chi,ndeg)
  !
