...
!------------------------------------------!
  ...
  subroutine lapack_dgesvd(tol,h)
    ...
    integer           :: info
    integer           :: nh1, nh2,i,Nkeep,j,nu1,nu2,nvt1,nvt2,LDVT,LDU
    integer           :: lwork
    ...
    !
    if (info/=0) then
      write (6,"(' lapack_dgesvd cannot allocate',i8)") info
      stop 'lapack_dgesvd cannot allocate'
    end if
    !
    ...
    !
    deallocate(work,u,vt,ut,v,s,stat=info)
    !
    if (info/=0) then
      write (6,"(' lapack_dgesvd cannot allocate',i8)") info
      stop 'lapack_dgesvd cannot allocate'
